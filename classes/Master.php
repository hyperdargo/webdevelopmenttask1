...
Class Master extends DBConnection {
	...
	function save_featured(){
		extract($_POST);
		$save = $this->conn->query("INSERT INTO `featured_game` set product_id = '{$product_id}' ");
		if($save){
			$this->settings->set_flashdata('success',"game successfully added to Featured Page.");
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
	...
	function delete_featured(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `featured_game` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"game successfully Removed from Featured List.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
