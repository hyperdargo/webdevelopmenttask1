...
<form action="" id="featured-form">
    <div class="form-group">
        <label for="product_id" class="control-label">Game</label>
        <select name="product_id" id="product_id" class="custom-select select2">
            <option value=""></option>
            <?php 
            $qry = $conn->query("SELECT * FROM `products` where id not in (SELECT product_id from `featured_game`) order by title desc ");
...
<script>
	$(document).ready(function(){
       $('.select2').select2({
           'placeholder':"Select Merach to Feature Here"
       })
